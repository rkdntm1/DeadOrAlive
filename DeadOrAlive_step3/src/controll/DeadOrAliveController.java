...
public class DeadOrAliveController {
	...
	public DeadOrAliveController() {
		...
		startFallDown(bomb, apple, displayer);
	}
	 ...
	 * @param bomb
	 */
	private static void startFallDown(Bomb bomb, Apple apple, DeadOrAliveWin displayer) {
		ScheduledJob job = new ScheduledJob(bomb, apple);
		...
		do {
			...
			// view 출력 
			displayer.display();
		} while (true); // 점수가 0점 보다 아래일 경우 게임 중지.
	}
...
class ScheduledJob extends TimerTask {
	...
	ScheduledJob(Bomb bomb, Apple apple){
		this.bomb = bomb;
		this.apple = apple;
	}
	public void run() {
