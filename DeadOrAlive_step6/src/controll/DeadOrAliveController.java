	...
	private static void startFallDown(Bomb bomb, Apple apple, Player player, DeadOrAliveWin displayer) {
		...
		do {
			...
			if (player.getScore() < 0) {
				// 재시작 안눌렀을경우 게임정지				
				if (!player.isRestart()) {
					player.setStart(false); // 떨어지지않도록 설정
				// 재시작버튼을 눌렀을 경우 점수, 사과와 폭탄 위치 초기화. 
				} else { 
					player.setScore(DeadOrAliveController.BASIC_SCORE); // 점수 초기화
					apple.reset();
					bomb.reset();
					player.setRestart(false); // 재시작버튼 다시 초기화
				}
			}
			// view 출력 
			displayer.display();
...
class ScheduledJob extends TimerTask {
	...
	public void run() {
		// 버튼이 눌러지면 실행
		if (player.isStart() || player.isRestart()) {
